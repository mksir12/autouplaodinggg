    ...
    for stream in audio_info["streams"]:
        ...
        if "channel_layout" in stream:
            ...
            if (
                ...
                in ffmpy_channel_layout_translation
            ):
...
def basic_get_missing_screen_size(
    torrent_info, is_disc, media_info_video_track, auto_mode, missing_value
):
...
def basic_get_missing_source(torrent_info, is_disc, auto_mode, missing_value):
    ...
    if is_disc and torrent_info["bdinfo"] is not None:
        if (
            "screen_size" in torrent_info
            and torrent_info["screen_size"] == "2160p"
        ):
...
def basic_get_missing_mediainfo(torrent_info, parse_me, mediainfo_file):
    ...
    if "largest_playlist" not in torrent_info:
        ...
        if "raw_video_file" in torrent_info:
            ...
            essential_path = (
                f"{os.path.basename(torrent_info['raw_video_file'])}"
            )
        else:
...
def basic_get_episode_basic_details(guess_it_result):
    ...
    if "season" not in guess_it_result:
        logging.error(
            "[BasicUtils] Could not detect the 'season' using guessit"
        )
        if (
...
def prepare_mediainfo_summary(media_info_result):
    ...
    for track in media_info_result["tracks"]:
        if track["track_type"] == "General":
            ...
            video[
                "Resolution"
            ] = f'{get(_this="sampled_width", _or="width", _from=track)}x{get(_this="sampled_height", _or="height", _from=track)}'
...
def basic_get_mediainfo_summary(media_info_result):
    ...
    ) = prepare_mediainfo_summary(media_info_result)
    meddiainfo_end_time = time.perf_counter()
    logging.debug(
    ...
    logging.info(
        f"[BasicUtils] Subtitle language codes: {subtitle_language_codes}"
    )

...
def generate_dot_torrent(
    ...
    torrent_title = normalize_for_system_path(torrent_title)
    ...
    if len(glob.glob(f"{working_dir}{hash_prefix}*.torrent")) == 0:
        ...
        if use_mktorrent:
            ...
            torrent_size = (
                sum(
                    f.stat().st_size
                    for f in Path(media).glob("**/*")
                    if f.is_file()
                )
                if os.path.isdir(media)
                else os.path.getsize(media)
            )
            ...
            logging.info(
                "[DotTorrentGeneration] Mktorrent .torrent write into {}".format(
                    "[" + source + "]" + torrent_title + ".torrent"
                )
            )

            logging.info(
                "[DotTorrentGeneration] Using torf to do some cleanup on the created torrent"
            )
            edit_torrent = GGBOTTorrent.read(
                glob.glob(
                    f"{working_dir}{hash_prefix}{tracker}-{torrent_title}.torrent"
                )[0]
            )
            ...
            if len(announce) > 1:
                ...
                for announce_url in announce:
                    edit_torrent.metainfo["announce-list"].append(
                        [announce_url]
                    )

            GGBOTTorrent.copy(edit_torrent).write(
                filepath=f"{working_dir}{hash_prefix}{tracker}-{torrent_title}.torrent",
                overwrite=True,
            )
        else:
            print("Using python torf to generate the torrent")
            torrent = GGBOTTorrent(
                media,
                trackers=announce,
                source=source,
            ...
            logging.info(
                "[DotTorrentGeneration] Trying to write into {}".format(
                    "[" + source + "]" + torrent_title + ".torrent"
                )
            )
    else:
        print(
            "Editing previous .torrent file to work with {} instead of generating a new one".format(
                source
            )
        )
        logging.info(
            "[DotTorrentGeneration] Editing previous .torrent file to work with {} instead of generating a new one".format(
                source
            )
        )
        ...
        edit_torrent = GGBOTTorrent.read(
            glob.glob(f"{working_dir}{hash_prefix}*.torrent")[0]
        )
        ...
        if len(announce) == 1:
            ...
            logging.debug(
                "[DotTorrentGeneration] Removing announce-list if present in existing torrent."
            )
    ...
    if os.path.isfile(
        ...
        logging.info(
            f"[DotTorrentGeneration] Successfully created the following file: {working_dir}{hash_prefix}{tracker}-{torrent_title}.torrent"
        )
    else:
        logging.error(
            f"[DotTorrentGeneration] The following .torrent file was not created: {working_dir}{hash_prefix}{tracker}-{torrent_title}.torrent"
        )

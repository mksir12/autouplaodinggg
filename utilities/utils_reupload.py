...
def is_un_processable_data_present_in_cache(info_hash, cache):
    """
...
def initialize_torrent_data(torrent, cache):
    logging.debug(
        f'[ReUploadUtils] Initializing torrent data in cache for {torrent["name"]}'
    )
    init_data = {
        "id": get_unique_id(),
        "hash": torrent["hash"],
...
def reupload_get_movie_db_from_cache(
    cache, cached_data, title, year, upload_type
):
...
def reupload_persist_updated_moviedb_to_cache(
    cache, movie_db, torrent_info, torrent_hash, original_title, original_year
):
...
def reupload_get_external_id_based_on_priority(
    ...
    # in case of tmdb id, we need to give the highest priority to the golden data obtained from the user via GG-BOT
    # Visor If bot wants tmdb id, and we have data in cached data (for currently uploading torrent) then we return it.
    # Otherwise, we go for the cached movieDB data (from another torrent) and finally we get the data from
    # media_info_summary
    if required_id == "tmdb":
...
def reupload_get_processable_torrents(torrent_client, cache):
    ...
    torrents = list(
        filter(
            lambda torrent: not is_un_processable_data_present_in_cache(
                torrent["hash"], cache
            ),
            filter(
...
def reupload_get_translated_torrent_path(torrent_path):
    if Environment.is_translation_needed():
        ...
        host_path = f"{Environment.get_uploader_accessible_path()}/".replace(
            "//", "/"
        )
        remote_path = f"{Environment.get_client_accessible_path()}/".replace(
            "//", "/"
        )
...
def update_success_status_for_torrent_upload(
    ...
    _save_job_repo_entry(
        cache, torrent["hash"], tracker, JobStatus.SUCCESS, upload_response
    )
    ...
    if (
        ...
        update_field(
            torrent["hash"], "status", TorrentStatus.SUCCESS, False, cache
        )
...
def update_failure_status_for_torrent_upload(
    ...
    # this is the first tracker for this torrent
    _save_job_repo_entry(
        cache, torrent["hash"], tracker, JobStatus.FAILED, upload_response
    )
    ...
    if (
        ...
        # updating the overall status of the torrent
        update_field(
            torrent["hash"], "status", TorrentStatus.FAILED, False, cache
        )

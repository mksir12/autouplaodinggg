...
def _make_request(url, method, search_site, site_name, json_data=None, multipart_data=None, headers=None):
    try:
...
def _fill_existing_release_types_with_source_data(existing_release_types, torrent_title, torrent_title_split, torrent_title_upper_split):
    # Bluray Encode
...
def search_for_dupes_api(tracker, search_site, imdb, tmdb, tvmaze, torrent_info, tracker_api, working_folder, auto_mode):
    ...
    #       1.b.a If and tracker torrent both are same (REPACK and REPACK) then thats a dupe.
        ...
        for existing_release_types_key in list(existing_release_types.keys()):
            ...
            if len(extracted_season_episode_from_title) == 3:
                ...
                if not is_full_season:
                    ...
                    cent_percent_dupes[existing_release_types_key] = "Season pack available"
    ...
    for possible_dupe_title in existing_release_types:
        ...
        if possible_dupe_title in cent_percent_dupes:
            ...
            possible_dupe_with_percentage_dict[possible_dupe_title] = _fuzzy_similarity(
                ...
                release_year=torrent_info["year"] if "year" in torrent_info else None,

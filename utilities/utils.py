...
def write_cutsom_user_inputs_to_description(torrent_info,
    ...
    if "custom_user_inputs" in torrent_info and torrent_info[
            "custom_user_inputs"] is not None:
            ...
            with open(description_file_path, 'a') as description:
                ...
                for custom_user_input in torrent_info["custom_user_inputs"]:
                    ...
                    if custom_user_input["key"] not in description_components:
                        ...
                        if "title" in custom_user_input and custom_user_input[
                                "title"] is not None:
                            logging.debug(
...
def add_bbcode_images_to_description(torrent_info, config,
                                     description_file_path, bbcode_line_break):
    screenshot_type = config[
        "screenshot_type"] if "screenshot_type" in config else None
...
def write_uploader_signature_to_description(description_file_path, tracker,
                                            bbcode_line_break, release_group):
    ...
    with open(description_file_path, 'a') as description:
        ...
        if Environment.get_uploader_signature() and len(
                Environment.get_uploader_signature()) > 0:
            logging.debug(
            ...
            if not uploader_signature.startswith(
                    "[center]") and not uploader_signature.endswith(
                        "[/center]"):
...
def delete_leftover_files(working_folder, file, resume=False):
    ...
    if Environment.is_readble_temp_data_needed():
        files = f'{file}/'.replace("//", "/").strip().replace(" ", ".").replace(
            ":", ".").replace("'", "").split("/")[:-1]
        files.reverse()
        unique_hash = files[0]
    else:
    ...
    if not Path(
            SCREENSHOTS_PATH.format(base_path=working_folder,
                                    sub_folder=unique_hash)).is_dir():
        os.mkdir(
            SCREENSHOTS_PATH.format(base_path=working_folder,
                                    sub_folder=unique_hash))
...
def get_and_validate_configured_trackers(trackers, all_trackers, api_keys_dict,
                                         all_trackers_list):
    upload_to_trackers = []
    # small sanity check
...
def _get_client_translated_path(torrent_info):
    ...
    if Environment.is_translation_needed():
        ...
        if "__MISCONFIGURED_PATH__/" in [
                client_accessible_path, uploader_accessible_path
        ]:
            logging.error(
...
def _post_mode_cross_seed(torrent_client, torrent_info, working_folder, tracker,
    ...
    if f"{tracker}_upload_status" in torrent_info and torrent_info[
            f"{tracker}_upload_status"] == True:
        logging.info(
        ...
        if allow_multiple_files == False and "raw_video_file" in torrent_info and torrent_info[
                "type"] == "movie":
            logging.info(
    ...
    for move_location_key, move_location_value in move_locations.items():
        ...
        if os.path.exists(move_location_value):
            ...
            if move_location_key == 'media':
                if str(f"{Path(torrent_info['upload_media']).parent}/"
                      ) == move_location_value:
                    console.print(
                    ...
                    if Environment.is_type_based_move_enabled():
                        ...
                        os.makedirs(os.path.dirname(move_location_value),
                                    exist_ok=True)
                    logging.info(
                        f"[Utils] Moving {torrent_info['upload_media']} to {move_location_value }"
                    )
                    try:
                        shutil.move(torrent_info["upload_media"],
                                    move_location_value)
                    except Exception:
                        logging.exception(
...
def perform_post_processing(torrent_info,
                            torrent_client,
                            working_folder,
                            tracker,
    ...
    if Environment.is_post_processing_needed():
        ...
        if not torrent_info["upload_media"].startswith("/"):
            torrent_info[
                "upload_media"] = f'{working_folder}/{torrent_info["upload_media"]}'
            logging.info(
        ...
        if post_processing_mode == "CROSS_SEED":
            console.print(
                ...
                justify="center")
            return _post_mode_cross_seed(torrent_client, torrent_info,
                                         working_folder, tracker,
                                         allow_multiple_files)
...
def sanitize_release_group_from_guessit(torrent_info):
    ...
    if "release_group" in torrent_info and len(
            torrent_info["release_group"]) > 0:
        logging.debug(
        ...
        upload_media = torrent_info["upload_media"][:-1] if torrent_info[
            "upload_media"].endswith("/") else torrent_info["upload_media"]
        # if title has spaces in it, then we remove them. [...H.264 - RELEASE_GROUP  ==> ...H.264-RELEASE_GROUP]
        upload_media = upload_media.replace(" ", "")
        if upload_media.replace(".mkv", "").replace(
                ".mp4", "").endswith(f"-{torrent_info['release_group']}"):
            # well the release group identified by guessit seems correct.
            logging.debug(
                f"[Utils] Release group identified by guessit: '{torrent_info['release_group']}' passed validation successfully"
            )
            ...
            return "DDH"
        elif torrent_info['release_group'] == "HorribleSubs":
            logging.info("[Utils] Hotfix for release group 'HorribleSubs'")
            return torrent_info['release_group']
        else:
            logging.debug(
                "[Utils] Couldn't validate the release group identified by guessit. Setting release group as NOGROUP"
            )
            return "NOGROUP"
    else:
        logging.debug(
    ...
    if technical_jargons["authentication_mode"] == "HEADER":
        if len(technical_jargons["headers"]) > 0:
            ...
            for header in technical_jargons["headers"]:
                ...
                headers[header["key"]] = tracker_api if header[
...
def _can_upload_to_ptp():
    ...
    while Environment.get_image_host_by_priority(enabled_img_host_num_loop +
                                                 1) is not None:
        if Environment.get_image_host_by_priority(enabled_img_host_num_loop +
                                                  1) == "ptpimg":
            found_ptpimg = True
            break
    ...
    if Environment.get_image_host_api_key("ptpimg") is None or len(
            Environment.get_image_host_api_key("ptpimg")) <= 0:
        return False
...
def validate_templates_in_path(template_dir, template_validator):
    valid_templates = list(
        map(
            ...
            filter(
                template_validator.
                is_valid,  # validating the template against the site_templates json schema.
                filter(
                    lambda entry: entry.is_file() and entry.suffix ==
...
def copy_template(valid_templates, source_dir, target_dir):
    ...
    for template in valid_templates:
        ...
        shutil.copy(str(Path(f"{source_dir}{template}.json")),
...
def validate_and_load_external_templates(template_validator, working_folder):
    ...
    total_number_templates = len(
        list(
            filter(lambda entry: entry.is_file() and entry.suffix == ".json",
    ...
    for entry in Path(external_templates_dir).glob('**/*'):
        if entry.is_file(
        ) and entry.suffix == ".json":  # we are only interested in .json files
    ...
    if len(valid_templates) > 0:
        if has_failed:
            console.print(
                "Please see log for more deatils regarding template validaion failures..."
            )
        ...
        copy_template(valid_templates, external_templates_dir,
                      temp_template_dir)
        return valid_templates, api_key_dict, {
...
def add_argument_tags(argument_tags):
    ...
    return None if argument_tags is None or len(argument_tags) == 0 else sorted(
        list(set(argument_tags)))

...
def is_auto_mode():
    return bool(os.getenv('auto_mode', False))

def is_force_auto_upload():
    return bool(os.getenv('force_auto_upload', False))

def is_check_dupes():
    return bool(os.getenv('check_dupes', False))

def is_containerized():
    return bool(os.getenv("IS_CONTAINERIZED", False))

def is_full_disk_supported():
    return bool(os.getenv("IS_FULL_DISK_SUPPORTED", False))

def is_live():
    return bool(os.getenv('live', False))

def is_readble_temp_data_needed():
    return bool(os.getenv("readable_temp_data", False))
...
def is_translation_needed():
    return bool(os.getenv('translation_needed', False))
...
def is_no_spoiler_screenshot():
    return bool(os.getenv("no_spoilers", False))
...
def is_post_processing_needed():
    return bool(os.getenv('enable_post_processing', False))

def is_type_based_move_enabled():
    return bool(os.getenv('enable_type_base_move', False))
...
def is_dynamic_tracker_selection_needed():
    return bool(os.getenv("dynamic_tracker_selection", False))

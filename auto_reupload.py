...
from modules.constants import (
    AUDIO_CODECS_MAP, BLURAY_REGIONS_MAP, COOKIES_DUMP_DIR,
    DESCRIPTION_FILE_PATH, MEDIAINFO_FILE_PATH, REUPLOADER_CONFIG,
    REUPLOADER_LOG, REUPLOADER_SAMPLE_CONFIG, SCENE_GROUPS_MAP,
    SCREENSHOTS_RESULT_FILE_PATH, SITE_TEMPLATES_DIR, STREAMING_SERVICES_MAP,
    STREAMING_SERVICES_REVERSE_MAP, TAG_GROUPINGS, TEMPLATE_SCHEMA_LOCATION,
    TRACKER_ACRONYMS, VALIDATED_SITE_TEMPLATES_DIR, WORKING_DIR)
# processing modules
...
common_args.add_argument(
    ...
    action='store_true',
    help=
    "Tf you want your upload to be anonymous (no other info needed, just input '-anon'"
)
...
uncommon_args.add_argument(
    ...
    action='store_true',
    help=
    "Force use PyMediaInfo to extract video codec over regex extraction from file name"
)
uncommon_args.add_argument(
    '-ss',
    '--skip_screenshots',
...
uncommon_args.add_argument(
    ...
    action='store_true',
    help=
    "Unsupported for AutoReuploader. Added for compatibility with upload assistant"
)
uncommon_args.add_argument(
    '-let',
    ...
    action='store_true',
    help=
    "When enabled uploader will load external site templates from ./external/site_templates location"
)
uncommon_args.add_argument('-tag',
                           '--tags',
                           nargs='*',
...
uncommon_args.add_argument(
    ...
    nargs=2,
    help=
    "Args: [source] [source_type] - Set a fallback source and source_type to releases whose source cannot be identified."
)
...
internal_args.add_argument('-featured',
                           action='store_true',
...
internal_args.add_argument('-sticky',
                           action='store_true',
...
valid_templates = utils.validate_templates_in_path(site_templates_path,
                                                   template_validator)
# copy all the valid templates to workdir.
utils.copy_template(
    valid_templates, site_templates_path,
...
if args.load_external_templates:
    ...
    valid_ext_templates, ext_api_keys_dict, ext_acronyms = utils.validate_and_load_external_templates(
...
def check_for_dupes_in_tracker(tracker, temp_tracker_api_key):
    ...
    config = json.load(
        open(site_templates_path +
             str(acronym_to_tracker.get(str(tracker).lower())) + ".json",
             encoding="utf-8"))
    ...
    try:
        ...
        logging.exception(
            f'[Main] Error occured while performing dupe check for tracker {tracker}. Error: {e}'
        )
        console.print(
            "[bold red]Unexpected error occured while performing dupe check. Assuming dupe exists on tracker and skipping[/bold red]"
        )
...
def upload_to_site(upload_to, tracker_api_key, config, tracker_settings):
    ...
    if config["technical_jargons"]["authentication_mode"] == "API_KEY":
        ...
        if len(config["technical_jargons"]["headers"]) > 0:
            ...
            for header in config["technical_jargons"]["headers"]:
                ...
                headers[header["key"]] = tracker_api_key if header[
                    "value"] == "API_KEY" else Environment.get_property_or_default(
    ...
    for key, val in tracker_settings.items():
        ...
        req_opt = 'Required' if key in config[
            "Required"] else 'Optional' if key in config[
        ...
        if str(config[req_opt][key]) == "file":
            ...
            if os.path.isfile(tracker_settings[key]):
                ...
                with open(tracker_settings[key]) as file_contents:
                    ...
                    payload[f'{key}[]'
                            ...
                            "MULTI-PART" else key] = screenshot_array
                    logging.debug(
            ...
            for line in tracker_settings[key].split("\n"):
                if len(line.strip()) > 0:
                    screenshot_array.append(line.strip())
            payload[f'{key}[]' if config["technical_jargons"]["payload_type"] ==
                    "MULTI-PART" else key] = screenshot_array
            logging.debug(
                f"[TrackerUpload] String array data for key '{key}' :: {screenshot_array}"
            )
    ...
    if response.status_code in (200, 201):
        ...
        if "success" in response.json():
            ...
            if str(response.json()["status"]).lower() == "true" or str(
                    response.json()["status"]).lower() == "success":
                logging.info(
                    f"[TrackerUpload] Upload to {upload_to} was a success!")
                console.line(count=2)
                console.rule(
...
def identify_type_and_basic_info(full_path, guess_it_result):
    ...
    keys_we_need_torrent_info = [
        'screen_size', 'source', 'audio_channels', 'type'
    ]
    ...
    if os.path.isdir(torrent_info["upload_media"]):
        ...
        if not str(torrent_info["upload_media"]).endswith('/'):
            torrent_info[
                "upload_media"] = f'{str(torrent_info["upload_media"])}/'
    ...
    # else:
    mediainfo_summary, tmdb, imdb, _, torrent_info[
        "subtitles"] = basic_utilities.basic_get_mediainfo_summary(
    ...
    columns_we_want = {
        ...
        'title':
            'Title',
        's00e00':
            f'{("Season" if len(torrent_info["s00e00"]) == 3 else "Episode") if "s00e00" in torrent_info else ""}',
        'year':
            f'{"Year" if "year" in torrent_info and torrent_info["type"] == "movie" else ""}',
        'source':
            'Source',
        'screen_size':
            'Resolution',
        'video_codec':
            'Video Codec',
        'hdr':
            f'{"HDR Format" if "hdr" in torrent_info else ""}',
        'dv':
            f'{"Dolby Vision" if "dv" in torrent_info else ""}',
        'audio_codec':
            'Audio Codec',
        'audio_channels':
            'Audio Channels',
        'atmos':
            f'{"Dolby Atmos" if "atmos" in torrent_info else ""}',
        'release_group':
            f'{"Release Group" if "release_group" in torrent_info else ""}'
    }
    ...
    codec_result_table = Table(box=box.SQUARE,
    ...
    for column_display_value in columns_we_want.values():
        if len(column_display_value) != 0:
            ...
            codec_result_table.add_column(f"{column_display_value}",
                                          justify='center',
                                          style='#38ACEC')
    ...
    for column_query_key, column_display_value in columns_we_want.items():
        if len(column_display_value) != 0:
            torrent_info_key_failsafe = (
                ...
                if column_query_key != 'type' else
                presentable_type) if column_query_key in torrent_info else None
            logging.debug(
...
def analyze_video_file(missing_value, media_info):
    ...
    if missing_value == "source":
        if "fallback_source" in torrent_info and torrent_info[
                "fallback_source"] is not None:
            logging.info(
                f"[Main] User has provided fallback source for uploads: {torrent_info['fallback_source']['source']}"
            )
    ...
    if missing_value == "screen_size":
        return basic_utilities.basic_get_missing_screen_size(
            torrent_info, False, media_info_video_track, auto_mode,
            missing_value)
    ...
    if missing_value == "audio_channels":
        ...
        return basic_utilities.basic_get_missing_audio_channels(
            torrent_info, False, auto_mode, parse_me, media_info_audio_track,
            missing_value)
...
def identify_miscellaneous_details(guess_it_result, file_to_parse):
    ...
    if "source_type" not in torrent_info:
        torrent_info[
            "source_type"] = miscellaneous_utilities.miscellaneous_identify_source_type(
                torrent_info["raw_file_name"], auto_mode,
                torrent_info["source"])

    # ------ WEB streaming service stuff here ------ #
    if torrent_info["source"] == "Web":
        # TODO check whether None needs to be set as `web_source`
        torrent_info["web_source"], torrent_info[
            "web_source_name"] = miscellaneous_utilities.miscellaneous_identify_web_streaming_source(
    ...
    # repacks
    torrent_info[
        "repack"] = miscellaneous_utilities.miscellaneous_identify_repacks(
            torrent_info["raw_file_name"])

    # --- Bluray disc type --- #
    if torrent_info["source_type"] == "bluray_disc":
        torrent_info[
            "bluray_disc_type"] = miscellaneous_utilities.miscellaneous_identify_bluray_disc_type(
    ...
    hdr_hybrid_remux_keyword_search = str(
        torrent_info["raw_file_name"]).lower().replace(" ", ".").replace(
            "-", ".").split(".")
    ...
    for word in hdr_hybrid_remux_keyword_search:
        ...
        if "dv" not in torrent_info or torrent_info["dv"] is None or len(
                torrent_info["dv"]) < 1:
    ...
    torrent_info[
        "edition"] = miscellaneous_utilities.miscellaneous_identify_bluray_edition(
            torrent_info["upload_media"])
    ...
    original_language = torrent_info["tmdb_metadata"][
        "original_language"] if torrent_info["tmdb_metadata"] is not None else ""
    dual, multi, commentary = miscellaneous_utilities.fill_dual_multi_and_commentary(
    ...
    torrent_info["container"] = os.path.splitext(
        torrent_info["raw_video_file"] if "raw_video_file" in
        torrent_info else torrent_info["upload_media"])[1]
    # Video continer information
...
def reupload_job():
    ...
    torrents = reupload_utilities.reupload_get_processable_torrents(
        torrent_client, cache)

    if torrents is None or len(torrents) == 0:
        logging.info(
            '[Main] There are no completed torrents for reuploading. Snoozing...'
        )
        return
    ...
    for torrent in torrents:
        ...
        logging.info(
            f"[Main] Trackers this torrent needs to be uploaded to are {upload_to_trackers_working}"
        )
        ...
        torrent_path = reupload_utilities.reupload_get_translated_torrent_path(
        ...
        torrent_info[
            "absolute_working_folder"] = f"{WORKING_DIR.format(base_path=working_folder)}{torrent_info['working_folder']}"
        ...
        if identify_type_and_basic_info(torrent_info["upload_media"],
                                        guess_it_result) == 'skip_to_next_file':
        ...
        movie_db = reupload_utilities.reupload_get_movie_db_from_cache(
            cache, cached_data, torrent_info["title"],
            torrent_info["year"] if "year" in torrent_info else "",
            torrent_info["type"])

        metadata_tmdb = reupload_utilities.reupload_get_external_id_based_on_priority(
            movie_db, torrent_info, cached_data, "tmdb")
        metadata_imdb = reupload_utilities.reupload_get_external_id_based_on_priority(
            movie_db, torrent_info, cached_data, "imdb")
        metadata_tvmaze = reupload_utilities.reupload_get_external_id_based_on_priority(
        ...
        possible_matches = metadata_utilities.fill_database_ids(
            torrent_info, [metadata_tmdb], [metadata_imdb], [metadata_tvmaze],
            auto_mode)

        if torrent_info["tmdb"] == "0" and torrent_info[
                "imdb"] == "0" and torrent_info["tvmaze"] == "0":
            ...
            reupload_utilities.update_field(
                torrent["hash"], "status",
                reupload_utilities.TorrentStatus.TMDB_IDENTIFICATION_FAILED,
                False, cache)
            reupload_utilities.update_field(torrent["hash"], "possible_matches",
        ...
        title, year, tvdb, mal = metadata_utilities.metadata_compare_tmdb_data_local(
            torrent_info)
        ...
        reupload_utilities.reupload_persist_updated_moviedb_to_cache(
            cache, movie_db, torrent_info, torrent["hash"], original_title,
            original_year)

        # -------- Fix/update values --------
        # set the correct video & audio codecs (Dolby Digital --> DDP, use x264 if encode vs remux etc)
        identify_miscellaneous_details(
            guess_it_result, torrent_info["raw_video_file"] if "raw_video_file"
            in torrent_info else torrent_info["upload_media"])
        ...
        if Environment.is_check_dupes() and len(
                upload_to_trackers_working) == 1:
            tracker = upload_to_trackers_working[0]
            temp_tracker_api_key = api_keys_dict[
                f"{str(tracker).lower()}_api_key"]

            console.line(count=2)
            console.rule(f"Dupe Check [bold]({tracker})[/bold]",
                         style='red',
                         align='center')
            ...
            if dupe_check_response:
                ...
                reupload_utilities.update_torrent_status(
                    torrent["hash"],
                    reupload_utilities.TorrentStatus.DUPE_CHECK_FAILED, cache)
                torrent_client.update_torrent_category(
                    info_hash=torrent["hash"],
                    category_name=reupload_utilities.TorrentStatus.
                    DUPE_CHECK_FAILED)
                console.print(
        ...
        media_info_duration = MediaInfo.parse(
            torrent_info["raw_video_file"] if "raw_video_file" in
            torrent_info else torrent_info["upload_media"]).tracks[1]
        ...
        is_screenshots_available = take_upload_screens(
            ...
            upload_media_import=torrent_info["raw_video_file"] if
            "raw_video_file" in torrent_info else torrent_info["upload_media"],
        ...
        if is_screenshots_available:
            ...
            torrent_info[
                "screenshots_data"] = SCREENSHOTS_RESULT_FILE_PATH.format(
                    base_path=working_folder,
                    sub_folder=torrent_info['working_folder'])
        ...
        for tracker in upload_to_trackers_working:
            ...
            torrent_info[
                "shameless_self_promotion"] = f'Uploaded with {"<3" if str(tracker).upper() in ("BHD", "BHDTV") or os.name == "nt" else "❤"} using GG-BOT Auto-ReUploader'
            ...
            if "banned_groups" in config and torrent_info[
                    "release_group"] in config["banned_groups"]:
                logging.fatal(
            ...
            if Environment.is_check_dupes(
            ) and len(upload_to_trackers_working) > 1:
                console.line(count=2)
                console.rule(f"Dupe Check [bold]({tracker})[/bold]",
                             style='red',
                             align='center')
            ...
            if torrent_info[
                    "type"] == "movie" and "raw_video_file" in torrent_info:
            ...
            if translation_utilities.choose_right_tracker_keys(
                    config, tracker_settings, tracker, torrent_info, args,
                    working_folder) == "STOP":
                continue
            ...
            # Tracker Settings
            if upload_status:
                reupload_utilities.update_success_status_for_torrent_upload(
                    cache, torrent, tracker, upload_response)
                ...
                reupload_utilities.update_failure_status_for_torrent_upload(
                    cache, torrent, tracker, upload_response)

        # marking dupe check failed torrents
        torrent_client.update_torrent_category(
            info_hash=torrent["hash"],
            category_name=None if is_non_dupes_present else
            reupload_utilities.TorrentStatus.DUPE_CHECK_FAILED)

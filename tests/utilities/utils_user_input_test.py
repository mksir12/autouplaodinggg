...
def test_collect_custom_messages_from_user(monkeypatch):
    ...
    here we'll add all components and validate the result.
    ...
    monkeypatch.setattr(
        ...
        lambda name, choices, default: next(prompt_answers),
    )
    ...
    monkeypatch.setattr(
        "rich.prompt.Confirm.ask", lambda name, default: next(confirm_answers)
    )

    expected = [
        {"key": "plain_text_code", "value": "Plain Text Data", "title": None},
        {
            "key": "spoiler_code",
            "value": "Spoiler Data",
            "title": "Spoiler Title",
        },
        {"key": "code_code", "value": "Code Data", "title": None},
    ]
    assert (
        collect_custom_messages_from_user(
            f"{working_folder}/tests/resources/custom_input/custom_text_components.json"
        )
        == expected
    )

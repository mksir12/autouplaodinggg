...
def run_around_tests():
    ...
    Path(f"{folder}/temp_upload/{utils.get_hash('some_name')}/screenshots"
    ...
    Path(f"{folder}/external/site_templates/").mkdir(
        parents=True, exist_ok=True)  # external site templates folder
    Path(f"{folder}/external/tracker/").mkdir(
        parents=True, exist_ok=True)  # external site acronym mapping location
    Path(f"{folder}/empty_dir/").mkdir(
        parents=True, exist_ok=True)  # empty dir for any tests that needs it

    # creating external dupe templates
    shutil.copy(f"{working_folder}/site_templates/blutopia.json",
                f"{folder}/external/site_templates/sample.json")
    shutil.copy(f"{working_folder}/site_templates/aither.json",
...
def test_delete_leftover_humanreadable_files(mocker):
    ...
    assert Path(f"{working_folder}{temp_working_dir}/temp_upload/{old_hash}"
               ).is_dir() == False
    assert Path(
...
def test_delete_leftover_humanreadable_files_disabled(mocker):
    ...
    assert Path(f"{working_folder}{temp_working_dir}/temp_upload/{new_hash}"
               ).is_dir() == True
    assert Path(f"{working_folder}{temp_working_dir}/temp_upload/{old_hash}"
               ).is_dir() == False
    assert Path(
...
def test_delete_leftover_files():
    ...
    assert Path(f"{working_folder}{temp_working_dir}/temp_upload/{hash}"
               ).is_dir() == True
    assert Path(f"{working_folder}{temp_working_dir}/temp_upload/{old_hash}"
               ).is_dir() == False
    assert Path(
...
def test_create_temp_upload_itself():
    ...
    hash = utils.get_hash("some_name1")
    assert computed_working_folder == f"{hash}/"
    assert Path(f"{working_folder}{temp_working_dir}/nothing/temp_upload/{hash}"
               ).is_dir() == True
    assert Path(
...
@pytest.mark.parametrize(("torrent_info", "expected"), [
    pytest.param(
        {
            "release_group":
                "RELEASE_GROUP",
            "upload_media":
                "/data/Movies/Name of the Movie 2022 STREAM WEB-DL DD+ 5.1 Atmos DV HDR HEVC-RELEASE_GROUP.mkv"
        },
        "RELEASE_GROUP",
    ...
    pytest.param(
        {
            "upload_media":
                "/data/Movies/Name of the Movie 2022 STREAM WEB-DL DD+ 5.1 Atmos DV HDR HEVC.mkv"
        },
        "NOGROUP",
    ...
    pytest.param(
        {
            "release_group":
                "X-RELEASE_GROUP",
            "upload_media":
                "/data/Movies/Name of the Movie 2022 STREAM WEB-DL 7.1 Atmos DV HDR HEV DTS-X-RELEASE_GROUP.mkv"
        },
        "RELEASE_GROUP",
    ...
    pytest.param(
        {
            "release_group":
                "RELEASE_GROUP",
            "upload_media":
                "/data/Movies/Miss Sadie Thompson 1953 1080p Blu-ray Remux AVC FLAC 2.0 - RELEASE_GROUP.mkv"
        },
        "RELEASE_GROUP",
    ...
    pytest.param(
        {
            "release_group":
                "640k-DDH",
            "upload_media":
                "/data/Movies/Bhramam.2021.HQ.2160p.AMZN.WEB.DL.HEVC.DDP5.1.640k-DDH.mkv"
        },
        "DDH",
    ...
    pytest.param(
        {
            "release_group":
                "HorribleSubs",
            "upload_media":
                "/downloads/[HorribleSubs] Otome Game no Hametsu Flag shika Nai Akuyaku Reijou ni Tensei Shiteshimatta [720p]/"
        },
        "HorribleSubs",
        id="release_group_HorribleSubs"),
])
...
def test_copy_template_with_already_existing_data():
    ...
    for template in valid_templates:
        ...
        json.dump(template_json_file,
                  open(str(Path(f"{target_dir}{template}.json")), "w"))

    utils.copy_template(valid_templates, source_dir, target_dir)
    for template in valid_templates:
        assert Path(f"{target_dir}{template}.json").is_file()
        assert json.load(open(str(
            Path(f"{target_dir}{template}.json"))))["name"] != "FAKE_NAME"
...
def test_validate_and_load_external_templates(mocker):
    ...
    valid_templates, api_key_dict, acronym_obtained = utils.validate_and_load_external_templates(
...
def test_validate_and_load_external_templates_no_dir():
    ...
    assert utils.validate_and_load_external_templates(
        template_validator,
        f"{working_folder}{temp_working_dir}/fake_dir") == ([], {}, {})


@pytest.mark.parametrize(("argument_tags", "expected"), [
    ...
    pytest.param(['tag1', 'tag2'], ['tag1', 'tag2'],
                 id="multiple_tags_provided"),
    pytest.param(['tag1', 'tag2', 'tag1'], ['tag1', 'tag2'],
                 id="dupes_in_tags"),
    pytest.param(['tag3', 'tag2', 'tag1'], ['tag1', 'tag2', 'tag3'],

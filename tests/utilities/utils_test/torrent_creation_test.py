...
def run_around_tests():
    ...
    if Path(folder).is_dir():
        ...
        Path(
            f"{folder}/temp_upload/{utils.get_hash('GenerateTorrentTesting')}"
        ).mkdir(
...
def test_folder_torrent_creation_pytorf():
    ...
    expected_torrent_title = f'{working_folder}{temp_working_dir}/temp_upload/{utils.get_hash("GenerateTorrentTesting")}/GG-BOT-{utils.normalize_for_system_path("This.Is.The.Title.Of.The.Torrent")}.torrent'
    assert Path(expected_torrent_title).is_file() == True

    created_torrent = Torrent.read(glob.glob(expected_torrent_title)[0])
    assert (
        created_torrent.comment == "Torrent created by GG-Bot Upload Assistant"
    )
    assert created_torrent.created_by == "GG-Bot Upload Assistant"
    assert (
        created_torrent.metainfo["announce"]
        == "https://gg-bot.com/announce/commence/testing"
    )
    assert (
        created_torrent.metainfo["comment"]
        == "Torrent created by GG-Bot Upload Assistant"
    )
...
def test_folder_torrent_creation_mktorrent():
    ...
    expected_torrent_title = f'{working_folder}{temp_working_dir}/temp_upload/{utils.get_hash("GenerateTorrentTesting")}/GG-BOT-{utils.normalize_for_system_path("This.Is.The.Title.Of.The.Torrent")}.torrent'
    assert (
        Path(expected_torrent_title).is_file() == True
...
def test_single_file_torrent_creation_pytorf_multiple_announce():
    announce_list = [
        "https://gg-bot.com/announce/commence/1",
        "https://gg-bot.com/announce/commence/2",
        "https://gg-bot.com/announce/commence/3",
    ]
    torrent_utilities.generate_dot_torrent(
        media=f"{working_folder}{data_dir}/file1/file1.dat",
...
def test_existing_torrent_modification_single_announce():
    ...
    # check whether the .torrent file has been created
    expected_torrent_title = f'{working_folder}{temp_working_dir}/temp_upload/{utils.get_hash("GenerateTorrentTesting")}/GG-BOT-TRACKER-TWO-{utils.normalize_for_system_path("This.Is.The.Title.Of.The.Torrent")}.torrent'
    assert Path(expected_torrent_title).is_file() == True
...
def test_existing_single_announce_torrent_modification_multiple_announce():
    ...
    announce_list = [
        "https://gg-bot.com/announce/edited/1",
        "https://gg-bot.com/announce/edited/2",
        "https://gg-bot.com/announce/edited/3",
    ]
    torrent_utilities.generate_dot_torrent(
        media=f"{working_folder}{data_dir}/file1/file1.dat",
        announce=announce_list,
...
def test_calculate_piece_size(input, expected):
    assert (
        torrent_utilities.calculate_piece_size(input) == expected
    )  # 1 GB => 1 MiB
